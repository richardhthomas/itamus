...
<div id="header2clininfo">
<h3 class="clininfo">Clinical Information</h3>
</div>


<?php

$page = "clininfo.php?topic=" . $_GET['topic'];
...
*/
echo "<div id='leftmenuclininfo'>
	<ul>
		<li><a ";
		if ($_GET['topic']=="All")
			echo "class='slctdclininfo' ";
		echo "href='clininfo.php?topic=All' target='_self'>All</a></li>
		<li><a ";
		if ($_GET['topic']=="General")
			echo "class='slctdclininfo' ";
		echo "href='clininfo.php?topic=General' target='_self'>General</a></li>";

$leftmenulist = mysql_query("SELECT DISTINCT category FROM links_db WHERE link_type LIKE '%clinical_info%' ORDER BY category");
...
while($menuitem = mysql_fetch_array($leftmenulist))
	...
	if ($menuitem['category']!="General")
		...
		if ($menuitem['category']==$_GET['topic'])
			echo "class='slctdclininfo' ";
		echo "href='clininfo.php?topic=" . $menuitem['category'] . "' target='_self'>" . $menuitem['category'] . "</a></li>";
		}
	}
...
{
if ($_GET['topic']=="All")
	$query = "SELECT link_name,link_destination,description,status FROM links_db WHERE link_type LIKE '%clinical_info%' ORDER BY link_name";
else
	$query = "SELECT link_name,link_destination,description,status FROM links_db WHERE link_type LIKE '%clinical_info%' AND category='" . $_GET['topic'] . "' ORDER BY link_name";
...
while($row = mysql_fetch_array($result))
  ...
  if ($row['status']!="x")
	...
	if ($row['status']=="b")
		...
		{
		echo "<td><a class='clininfo' href='" . $row['link_destination'] . "' onClick=\"_gaq.push(['_trackEvent', 'clinical_info', '" . $row['link_name'] . "']);\">" . $row['link_name'] . "</a></td>";
