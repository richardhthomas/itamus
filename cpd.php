...
<div id="header2cpd">
<h3 class="cpd">Continuing Professional Development</h3>
</div>
...
// Build left menu... there isn't one yet...
	
// Build the table

$query = "SELECT link_name,link_destination,description,status FROM links_db WHERE link_type LIKE '%cpd%' ORDER BY link_name";
...
while($row = mysql_fetch_array($result))
  ...
  if ($row['status']!="x")
	...
	if ($row['status']=="b")
		...
		{
		echo "<td><a class='cpd' href='" . $row['link_destination'] . "' onClick=\"_gaq.push(['_trackEvent', 'cpd', '" . $row['link_name'] . "']);\">" . $row['link_name'] . "</a></td>";
